    ...
    chrome.tabs.sendMessage(
      ...
      function (response) {
        ...
        if (
          ...
          onFail()
          setTimeout(function () {
            communication_test(onFail, onSuccess, retryTimeMiliSeconds)
          }, retryTimeMiliSeconds)
        } else {
          onSuccess()
        }
      }
...
class Editor {
  ...
  make_button_active(filename) {
    ...
    Array.from(all_html_elements).forEach(function (element) {
      ...
      if (element.value === filename) {
        ...
        all_close_html_elements[
          Array.from(all_html_elements).indexOf(element)
        ].style.opacity = 1
      }
  ...
  resize_open_file_html_elements() {
    ...
    let button_width = Math.round(
      (parseInt(editor_width_str.substring(0, editor_width_str.length - 2)) -
        20) /
        all_html_elements.length
    )
  ...
  save_file_by_name(filename) {
    ...
    current_file_data.active_websites =
      this.navigator.nav_items[index].active_websites
    ...
    current_file_data.reload_on_remove =
      this.navigator.nav_items[index].reload_on_remove === undefined
        ? false
        : this.navigator.nav_items[index].reload_on_remove
  ...
  save_current_file() {
    ...
    let position =
      this.navigator.position_selection.options[
        this.navigator.position_selection.selectedIndex
      ].value
    let mode =
      this.navigator.mode_selection.options[
        this.navigator.mode_selection.selectedIndex
      ].value
    let all_new_data = {}
    ...
    current_file_data.reload_on_remove =
      reload_on_remove === undefined ? false : reload_on_remove
...
class File {
  constructor(
    ...
    this.reload_on_remove =
      reload_on_remove === undefined ? false : reload_on_remove // Determines if the page should reload after removing the manipulation.
...
class Navigation {
  constructor() {
    ...
    this.enabled_sites_text_area = document.getElementById(
      'enabled-sites-text-area'
    )

    // Checkboxes
    this.active_checkbox = document.getElementById('active-checkbox')
    this.reload_on_remove_checkbox = document.getElementById(
      'reload-on-remove-checkbox'
    )
    ...
    this.active_websites_label = document.getElementById(
      'active-websites-label'
    )
    ...
    this.language_selection_label = document.getElementById(
    ...
    this.project_support_label = document.getElementById(
      'project-support-label'
    )

    // Filename editing items before editing.
    this.filename_label = document.getElementById('filename-label')
    this.change_filename_not_editing_div = document.getElementById(
      'change-filename-not-editing-div'
    )
    this.change_filename_label = document.getElementById(
      'change-filename-label'
    )
    this.change_filename_button = document.getElementById(
      'change-filename-button'
    )

    // Filename editing items while editing.
    this.change_filename_editing_div = document.getElementById(
      'change-filename-editing-div'
    )
    this.change_filename_textfield = document.getElementById(
      'change-filename-textfield'
    )
    this.change_filename_button_div = document.getElementById(
      'change-filename-button-div'
    )
    this.cancel_filename_change_button = document.getElementById(
      'cancel-filename-change-button'
    )
    this.save_filename_change_button = document.getElementById(
      'save-filename-change-button'
    )

    // Zoom factor elements.
    this.zoom_out_button = document.getElementById('zoom-out-button')
    this.zoom_percentage_label = document.getElementById(
    ...
    this.main_menu_division_lines = document.getElementsByClassName(
      'main-menu-division-line'
    )
    this.editor_menu_division_lines = document.getElementsByClassName(
      'editor-menu-division-line'
    )
    ...
    this.main_nav_items = [
      ...
      this.info_text,
      this.info_button,
      this.bug_report_button,
      this.language_selection_label,
      this.donate_button,
      this.extra_info_label,
      this.project_support_label,
    ].concat(Array.from(this.main_menu_division_lines))

    this.editor_menu_items = [
      this.back_div,
      this.try_button,
      this.active_websites_label,
      this.enabled_sites_text_area,
      this.matching_pages_label,
      this.mode_selection,
      this.delete_button,
      this.filename_label,
      this.change_filename_not_editing_div,
      this.change_filename_editing_div,
    ].concat(Array.from(this.editor_menu_division_lines))
  ...
  bind_html_elements() {
    ...
    this.info_button.onclick = function () {
      window
        .open('https://github.com/Ruud14/Page-Manipulator', '_blank')
        .focus()
    }.bind(this)
    ...
    this.bug_report_button.onclick = function () {
      window
        .open('https://github.com/Ruud14/Page-Manipulator/issues', '_blank')
        .focus()
    }.bind(this)
    ...
    this.active_checkbox.onchange = function () {
      for (let element of this.nav_items) {
        if (element.filename === this.editor.active_file) {
          ...
          if (
            this.nav_items[index].active_websites.replaceAll(/\s/g, '') ===
              '' &&
            this.active_checkbox.checked === true
          ) {
            alert(
              "You must first specify the active websites in the 'active websites' textarea. If you want this manipulation to be active on all webistes, put 'all' into the 'active websites' textarea."
            )
    ...
    let position_change_function = function () {
      for (let element of this.nav_items) {
        if (element.filename === this.editor.active_file) {
          let index = this.nav_items.indexOf(element)
          this.nav_items[index].position =
            this.position_selection.options[
              this.position_selection.selectedIndex
            ].value
          this.editor.save_current_file()
          break
        }
    ...
    let mode_change_function = function () {
      for (let element of this.nav_items) {
        if (element.filename === this.editor.active_file) {
          let index = this.nav_items.indexOf(element)
          this.nav_items[index].mode =
            this.mode_selection.options[this.mode_selection.selectedIndex].value
          this.editor.save_current_file()
          break
        }
      }
    ...
    let active_sites_textarea_change_function = function () {
      for (let element of this.nav_items) {
        if (element.filename === this.editor.active_file) {
          let index = this.nav_items.indexOf(element)
          this.nav_items[index].active_websites =
      ...
      for (let element of this.nav_items) {
        if (element.filename === this.editor.active_file) {
          ...
          this.nav_items[index].active_websites =
            this.enabled_sites_text_area.value
          if (
            this.enabled_sites_text_area.value.replaceAll(/\s/g, '') === '' &&
            this.active_checkbox.checked === true
          ) {
    ...
    this.try_button.onclick = function () {
      ...
      let position =
        this.position_selection.options[this.position_selection.selectedIndex]
          .value
    ...
    this.remove_try_button.onclick = function (e) {
      ...
      if (e.target !== this.remove_try_button) return
    ...
    this.reload_on_remove_checkbox.onchange = function () {
      for (let element of this.nav_items) {
        if (element.filename === this.editor.active_file) {
          let index = this.nav_items.indexOf(element)
          this.nav_items[index].reload_on_remove =
            this.reload_on_remove_checkbox.checked
          this.editor.save_current_file()
          break
        }
      }
    ...
    this.delete_button.onclick = function () {
      ...
      if (
        confirm(
          'Are you sure you want to delete ' + this.editor.active_file + '?'
        )
      ) {
        chrome.tabs.query(
          { active: true, currentWindow: true },
          function (tabs) {
            ...
            let index = this.get_nav_item_index_by_filename(
              this.editor.active_file
            )
    ...
    this.make_button.onclick = function () {
      ...
      if (this.file_exists(filename)) {
        ...
        if (
          this.current_menu === 'JS' ||
          this.current_menu === 'CSS' ||
          this.current_menu === 'HTML'
        ) {
          // Add the correct file extension if there isn't one already.
  ...
  file_exists(filename) {
    ...
    if (filename.endsWith(file_extension)) {
      ...
      // Check if there alreay is a file with the same name.
      if (
        this.get_nav_item_index_by_filename(filename + file_extension) == null
      ) {
        return false
      } else {
        return true
  ...
  set_zoom_factor(factor) {
    if (factor >= 0) {
      ...
      this.editor.editor_element.style['min-width'] =
        editor_width.toString() + 'px'
      this.editor.editor_element.style['min-height'] =
        editor_height.toString() + 'px'
      this.editor.editor.resize()
      this.navbar.style['min-height'] = nav_bar_height.toString() + 'px'
      this.zoom_percentage_label.innerText =
        (this.current_zoom_level + 100).toString() + ' %'
    }
  ...
  reload_nav_buttons() {
    // Clear the ul's.
    let all_js_file_html_elements = Array.from(
      document.getElementsByClassName('saved-js-nav-button')
    )
    let all_css_file_html_elements = Array.from(
      document.getElementsByClassName('saved-css-nav-button')
    )
    let all_html_file_html_elements = Array.from(
      document.getElementsByClassName('saved-html-nav-button')
    )
    let all_file_html_elements = all_js_file_html_elements
      .concat(all_css_file_html_elements)
      .concat(all_html_file_html_elements)
  ...
  load_open_files() {
    ...
    for (let element of this.nav_items) {
      ...
      if (
        element.open &&
        this.editor.files.length < this.editor.max_open_files
      ) {
  ...
  enable_menu_of_kind(kind) {
    ...
    switch (kind) {
      ...
      case 'HTML':
        ...
        let all_elements = document.getElementsByClassName(
          'saved-' + kind.toLowerCase() + '-nav-button'
        )
      ...
      case 'EDITOR':
        ...
        this.menu_title_label.innerText = kind_to_language(
          filename_to_kind(this.editor.active_file)
        )
        ...
        chrome.tabs.query(
          ...
          function (tabs) {
            ...
            chrome.tabs.sendMessage(
              tabs[0].id,
              { todo: 'getStatus', value: this.editor.active_file },
              function (response) {
                if (response != null) {
        ...
        for (let i = 0; i < positoin_options.length; i++) {
          if (
            positoin_options[i].value ===
            String(this.nav_items[index].position).toLowerCase()
          ) {
        ...
        for (let i = 0; i < mode_options.length; i++) {
          if (this.nav_items[index].mode != null) {
            if (
              mode_options[i].value ===
              String(this.nav_items[index].mode).toLowerCase()
            ) {
